...
def extraer_datos_entrenamiento(ruta_archivo):
    datos_por_epoca = {}
    patron = re.compile(
        r"Epoch (\d+), Step \d+: loss = (-?\d+(?:\.\d+)?), psnr = (-?\d+(?:\.\d+)?) dB"
    )
    ...
    with open(ruta_archivo, "r", encoding="utf-8") as archivo:
        for linea in archivo:
            ...
            if coincidencia:
                ...
                datos_por_epoca[epoca] = {"loss": loss, "psnr": psnr}
    return datos_por_epoca


# 1. Extraer los datos del archivo
archivo_log = input("Ingrese el nombre del archivo de log: ")
...
plt.plot(
    ...
    linestyle="-",
    alpha=0.6,
    label="Loss (Interpolado)",
)
# Superponemos los puntos reales
plt.scatter(
    epocas_reales, losses_reales, color="darkred", zorder=5, label="Datos Reales"
)
