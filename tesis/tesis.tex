...
La organización del documento es la siguiente. En la Sección~II se presentan los fundamentos teóricos de MAXIM y de MoE. En la Sección~III se detalla la metodología implementada: arquitectura, datos, protocolo de entrenamiento y métricas. La Sección~IV reporta los resultados cuantitativos de las tres configuraciones entrenadas y el análisis del ruteador. Las Secciones~V y~VI discuten los hallazgos y presentan las conclusiones, y la Sección~VII delinea el trabajo futuro.
...
Se entrenaron tres configuraciones que comparten infraestructura, hiperparámetros base y definición de las métricas. Las dos que operan sobre la mezcla completa ---MAXIM multi-tarea y MAXIM+MoE--- comparten exactamente el mismo conjunto de validación, por lo que son directamente comparables; la versión mono-tarea se valida sobre una sola tarea y se incluye como punto de referencia adicional:
\begin{itemize}
    ...
    \item \textbf{MAXIM multi-tarea}: el modelo base (variante S-3), sin MoE, entrenado sobre la mezcla de las cinco tareas e inicializado desde un \textit{checkpoint} preentrenado de MAXIM (Enhancement/LOL);
    \item \textbf{MAXIM+MoE}: la variante propuesta, cuyo backbone es la variante S-2, entrenada sobre la misma mezcla de cinco tareas pero desde inicialización aleatoria.
\end{itemize}

Las variantes S-2 y S-3 de MAXIM se diferencian en el número de etapas de procesamiento (dos y tres, respectivamente): a mayor número de etapas, mayor capacidad del modelo y más puntos de supervisión. Conviene notar que la variante MoE comparte tanto la escala (S-2) como la inicialización aleatoria con la línea base mono-tarea, lo que permite aislar el efecto del esquema de expertos y de la supervisión de una sola escala.
...
La calidad de reconstrucción se cuantifica con la \emph{PSNR} (\textit{Peak Signal-to-Noise Ratio}) sobre el conjunto de validación, calculada de manera idéntica en las tres configuraciones a partir del error cuadrático medio en escala $[0,255]$,
\begin{equation}
...
Se reportan los resultados de las tres configuraciones descritas en la metodología, todas entrenadas y evaluadas con el mismo protocolo. El análisis se centra en la PSNR de validación, en la dinámica de entrenamiento de la variante MoE y en el comportamiento de su ruteador.

\subsection{Resultados cuantitativos}

La Tabla~\ref{tab:cuant} resume el desempeño de las tres configuraciones, y la Fig.~\ref{fig:valpsnr} muestra la evolución de la PSNR de validación a lo largo del entrenamiento. Las dos versiones sin MoE convergen al rango esperado para MAXIM: la versión multi-tarea, beneficiada por la inicialización preentrenada, alcanza $25{,}5$~dB, mientras que la versión mono-tarea, entrenada desde cero, llega a $23{,}1$~dB con una curva más ruidosa pero claramente ascendente.

La variante MAXIM+MoE presenta un comportamiento marcadamente distinto. Tras una mejora inicial muy rápida durante las primeras tres épocas ---en las que pasa de $\sim\!10{,}5$ a $\sim\!12{,}3$~dB---, la PSNR de validación se estanca y oscila en torno a $12{,}5$--$12{,}7$~dB sin progreso apreciable. El mejor \textit{checkpoint} alcanza $12{,}7$~dB, una diferencia de casi $13$~dB respecto del modelo base multi-tarea, que se evalúa sobre el mismo conjunto de validación. Prolongar el entrenamiento más allá de las $60$ épocas no modificó este estancamiento, lo que descarta que se trate de un presupuesto de cómputo insuficiente. La versión mono-tarea, aunque se evalúa sobre una sola tarea, ofrece un contraste adicional útil: entrenada también desde cero, supera los $20$~dB ya en la época~6, mientras que la variante MoE nunca despega de la franja de los $12$--$13$~dB.

\begin{table}[ht]
\caption{\label{tab:cuant}Comparación cuantitativa sobre el conjunto de validación (parches de $256\times 256$). Se reporta la mejor PSNR alcanzada, la SSIM asociada y la pérdida L1 de validación. ``Escala'' indica la variante de MAXIM (S-2 o S-3, según el número de etapas); ``Inic.'' la inicialización (aleatoria o desde \textit{checkpoint} preentrenado) y ``Sup.'' el tipo de supervisión (multi-escala o de una sola escala). Los valores de SSIM, marcados con ``---'', se obtienen ejecutando el \textit{script} de evaluación \texttt{eval\_metrics.py} sobre los \textit{checkpoints} guardados.}
\begin{ruledtabular}
\begin{tabular}{lcccccc}
Modelo & Escala & Inic. & Sup. & PSNR (dB) & SSIM & L1 val. \\
\hline
MAXIM mono-tarea   & S-2 & aleat.   & multi  & $23{,}1$ & --- & $0{,}068$ \\
MAXIM multi-tarea  & S-3 & preentr. & multi  & $\mathbf{25{,}5}$ & --- & $\mathbf{0{,}042}$ \\
MAXIM+MoE          & S-2 & aleat.   & simple & $12{,}7$ & --- & $0{,}193$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\caption{\label{fig:valpsnr}Evolución de la PSNR de validación durante el entrenamiento, para un presupuesto común de $30$ épocas. Las dos versiones de MAXIM sin MoE mejoran de forma sostenida, mientras que la variante MAXIM+MoE se estanca tempranamente muy por debajo de ambas. La curva mono-tarea comienza en la época~6 por el rango disponible en el registro de entrenamiento.}
\end{figure}
...
\caption{\label{fig:cualit}Comparación cualitativa sobre muestras de validación de distintas tareas. De izquierda a derecha: entrada degradada, MAXIM multi-tarea, MAXIM+MoE e imagen de referencia. La figura se genera con el \textit{script} \texttt{eval\_metrics.py} a partir de los \textit{checkpoints} guardados.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_qualitative_enhance.pdf}
\caption{\label{fig:cualit_enh}Comparación cualitativa sobre la tarea \textit{enhance}, la única en la que el modelo mono-tarea es directamente comparable. De izquierda a derecha: entrada degradada, MAXIM mono-tarea, MAXIM multi-tarea, MAXIM+MoE e imagen de referencia. La figura se genera con el \textit{script} \texttt{eval\_metrics.py} a partir de los \textit{checkpoints} guardados.}
\end{figure}

\section{Discusión}

Los resultados indican que, en la configuración evaluada, la variante MAXIM+MoE no mejora a la versión sin MoE; por el contrario, queda muy por debajo de ambas referencias. Conviene, antes que nada, separar lo que la comparación permite concluir de lo que no. La diferencia frente al modelo multi-tarea está parcialmente confundida por dos factores: la inicialización (ese modelo partió de pesos preentrenados de MAXIM, mientras que la variante MoE se entrenó desde cero) y la escala del backbone (el multi-tarea emplea la variante S-3, de mayor capacidad, frente a la S-2 del MoE). Por eso la comparación más limpia es contra la versión mono-tarea, que comparte con el MoE tanto la escala (S-2) como la inicialización aleatoria. Aun así, esa versión superó los $20$~dB en la época~6 y siguió mejorando, mientras que la variante MoE se estancó por debajo de los $13$~dB desde la época~3. La diferencia, entonces, no se explica ni por el tamaño del modelo ni por el preentrenamiento: el cuello de botella está en el propio diseño y entrenamiento de la variante MoE.

El análisis del ruteador (Fig.~\ref{fig:moedyn}) apunta a un \emph{colapso hacia el ruteo uniforme}. Identificamos cuatro factores que, en conjunto, explican el comportamiento observado:
...
\begin{enumerate}
    ...
    \item \textbf{Condiciones de optimización restrictivas.} El tamaño de \textit{batch} de $2$, impuesto por la memoria disponible, produce estimaciones de gradiente ruidosas, lo que agrava la dificultad de entrenar un backbone profundo desde cero junto a un ruteador inestable.
\end{enumerate}
...
Como subproducto, el trabajo deja una línea base sólida y reproducible: una versión multi-tarea de MAXIM que, a partir de una inicialización preentrenada, alcanza $25{,}5$~dB sobre la mezcla de cinco tareas, y una versión mono-tarea de referencia. Ambas resultan útiles como punto de comparación para futuras variantes.

\section{Conclusiones}

El objetivo general de este trabajo fue evaluar si dotar a MAXIM de un esquema de Mixture of Experts en la etapa de reconstrucción aporta frente a una versión sin MoE. La evidencia experimental responde de forma negativa para la configuración estudiada: la variante MAXIM+MoE se estanca alrededor de $12{,}7$~dB de PSNR de validación, frente a los $23{,}1$ y $25{,}5$~dB de las versiones mono-tarea y multi-tarea sin MoE.
